...
{
public:
        server(std::string_view name, uint16_t port, servers *srvs, time_t now)
...
public:
        ...
        void add_servers(const std::string_view &delimited_servers, uint16_t port)
        {
...
fprintf(stderr, "unpersisting '%s'\n", fpath);
        ...
        {
                uint32_t loops = 0;
                while (dns_active(m_dns_context))
                {
        ...
        void a4_resolved(struct dns_rr_a4 *result, server *srv)
        {
                ...
                std::string_view host = result->dnsa4_cname;
...
private:
        ...
        void parse_delimited(const std::string_view& buff, const std::string_view& delims, uint16_t port)
        {
